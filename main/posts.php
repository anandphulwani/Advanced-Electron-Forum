...
global $categories, $forums, $active, $anonymous, $guests, $activeusers, $board, $poll, $user_group, $post_group, $tid, $tpg, $fid, $title, $topic, $post, $topicpages, $pg, $attachments,$page, $tree;
		...
		if(mysql_num_rows($qresult) > 0){
			...
			for($i = 1; $i <= mysql_num_rows($qresult); $i++){
				...
				if($row['atmt_isimage'] && $globals['attachmentshowimage']){
					...
					$atimg = '<img src="'.$globals['attachmenturl'].'/'.$row['at_file'].'" width="'.$row['at_width'].'" height="'.$row['at_height'].'" /><br />';	
				...
				$attachments[$row['at_pid']][] = $atimg.'<img src="'.$globals['url'].'/mimetypes/'.(empty($row['atmt_icon']) ? 'unknown.png' : $row['atmt_icon']).'" />&nbsp;<a href="'.$globals['index_url'].'act=downloadattach&atid='.$row['atid'].'">'.$row['at_original_file'].'</a> ('.$at_size.' KB, '.$l['downloads'].': '.$row['at_downloads'].')';
