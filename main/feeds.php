...
global $categories, $forums;
	...
	if(isset($_GET['topic']) && is_numeric(trim($_GET['topic']))){
		...
		if(mysql_num_rows($qresult) > 0){
			...
			$link = $globals['index_url'].'tid='.$tid;
			...
			if(in_array($topic['t_bid'], $visible_forums)){
				...
				if(!empty($board['rss_topic'])){
					...
					for($p = 0; $p < mysql_num_rows($qresult); $p++){
						...
						$feeds[$row['pid']]['link'] = $globals['index_url'].'tid='.$tid.'&amp;tpg='.$row['page'].'#p'.$row['pid'];//Link - Becareful of HTML Entities
	...
	//The Recent RSS Feeds for a Single Forum
		...
		if(!empty($fid) && in_array($fid, $visible_forums)){
			...
			$link = $globals['index_url'].'fid='.$fid;
			...
			if(!empty($board['rss'])){
				...
				for($p = 0; $p < mysql_num_rows($qresult); $p++){
					...
					$feeds[$row['pid']]['link'] = $globals['index_url'].'tid='.$row['tid'].'&amp;tpg='.$row['last_page'].'#p'.$row['pid'];//Link - Becareful of HTML Entities
