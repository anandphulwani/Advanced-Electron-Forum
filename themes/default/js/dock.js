...
function dock(){
	...
	this.dockify = function(){
		var self = this;
		for (i=0; i<img.length; i++){
			img[i].now.left = ((this.step-1)*img[i].now.left+img[i].target.left)/this.step;
			img[i].style.left = Math.floor(img[i].now.left)+'px';
	
			img[i].now.top = ((this.step-1)*img[i].now.top+img[i].target.top)/this.step;
			img[i].style.top = Math.floor(img[i].now.top)+'px';
	
			img[i].now.size = ((this.step-1)*img[i].now.size+img[i].target.size)/this.step;
