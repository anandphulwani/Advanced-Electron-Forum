...
global $categories, $forums, $active, $anonymous, $guests, $mostactive, $mostactive_ever, $inboards, $latest_mem, $active, $activebots, $pm, $onload, $dmenus, $online_today, $recent_posts, $user_groups;
	...
	if(!empty($online_today)){
		...
		foreach($online_today as $k => $v){
			...
			$users_today[] = '<a href="'.$globals['index_url'].'mid='.$online_today[$k]['id'].'" style="color: '.$online_today[$k]['mem_gr_colour'].';" >'.$online_today[$k]['username'].'</a>';
	...
	if(!empty($user_groups)){
		...
		foreach($user_groups as $k => $v){
			...
			$user_groups[$k] =  '<a href="'.$globals['index_url'].'act=members&group='.$k.'" style="color: '.$user_groups[$k]['mem_gr_colour'].';" >'.$user_groups[$k]['mem_gr_name'].'</a>';
	...
	<td class="cbgl"></td>
	<td class="cbg">
	<a href="'.$globals['index_url'].'#cid'.$categories[$c]['cid'].'" name="cid'.$categories[$c]['cid'].'">'.$categories[$c]['name'].'</a>
	</td>
	<td class="cbg" align="right">
	<a href="javascript:slider.slide(\'cat'.$c.'\')" ><img id="icat'.$categories[$c]['cid'].'" src="'.$theme['images'].'expanded.gif" /></a>
	</td>	
	<td class="cbgr"></td>
	</tr>
	</table>
	...
	<img src="'.(($forums[$c][$f]['fimage']) ? $forums[$c][$f]['fimage'] : (($forums[$c][$f]['is_read']) ? $theme['images'].'not.png' : $theme['images'].'new.png')).'">
	</td>
	
	<td class="mifor">
	<font class="forlink">
	<a href="'.(($forums[$c][$f]['fredirect']) ? $forums[$c][$f]['fredirect'] : $globals['index_url'].'fid='.$forums[$c][$f]['fid'] ).'">'.$forums[$c][$f]['fname'].'</a>
	</font><br />
	<font class="desc">
	...
	if(isset($forums[$c][$f]['moderators'])){
		...
		foreach($forums[$c][$f]['moderators'] as $mk => $mv){
			...
			$boardmoderators[$f][] = '<a href="'.$globals['index_url'].'mid='.$forums[$c][$f]['moderators'][$mk]['id'].'">'.$forums[$c][$f]['moderators'][$mk]['username'].'</a>';
	...
	if(isset($forums[$c][$f]['in_boards'])){					
		...
		foreach($forums[$c][$f]['in_boards'] as $ik => $iv){
			...
			$inboards[$f][] = (!empty($iv['is_read']) ? '' : '<b>').'<a href="'.$globals['index_url'].'fid='.$forums[$c][$f]['in_boards'][$ik]['fid'].'">'.$forums[$c][$f]['in_boards'][$ik]['fname'].'</a>'.(!empty($iv['is_read']) ? '' : '</b>');
	...
	if(empty($forums[$c][$f]['tid'])){
		...
		$l['in'].' <a href="'.$globals['index_url'].'tid='.$forums[$c][$f]['tid'].'" title="'.$l['go_to_first_post'].'">'.$forums[$c][$f]['topic'].'</a>&nbsp;&nbsp;&nbsp;<a href="'.$globals['index_url'].'tid='.$forums[$c][$f]['tid'].'&tpg='.$forums[$c][$f]['last_page'].'#p'.$forums[$c][$f]['pid'].'" title="'.$l['go_to_last_post'].'"><img src="'.$theme['images'].'right.gif" /></a><br />'.
		$l['by'].' '.(empty($forums[$c][$f]['username']) ? (empty($forums[$c][$f]['gposter_name']) ? $l['guest'] : $forums[$c][$f]['gposter_name']) : '<a href="'.$globals['index_url'].'mid='.$forums[$c][$f]['poster_id'].'">'.$forums[$c][$f]['username'].'</a>');
...
<tr>
<td align="left">
<img src="'.$theme['images'].'/new.png" alt="" border="0" align="middle" /> '.$l['new_posts'].' &nbsp;&nbsp;&nbsp;<img src="'.$theme['images'].'/not.png" alt="" border="0" align="middle" /> '.$l['no_new_posts'].'
</td>
<td align="right">'.($logged_in ? '<a href="'.$globals['index_url'].'act=markread&mark=board">'.$l['mark_board_read'].'</a>' : '').'
</td>
</tr>
</table>';
...
foreach($recent_posts as $rk => $rv){
	...
	echo '('.$rv['pdate'].') <b><a href="'.$globals['index_url'].'tid='.$rv['tid'].'&tpg='.$rv['last_page'].'#p'.$rv['pid'].'">'.$rv['topic'].'</a></b> '.$l['by'].' <a href="'.$globals['index_url'].'mid='.$rv['id'].'">'.$rv['username'].'</a>   (<a href="'.$globals['index_url'].'fid='.$rv['fid'].'">'.$rv['fname'].'</a>)<br />';
...
echo '<tr>
<td colspan="2" class="cbg1" align="left">
'.($user['view_active'] ? '<a href="'.$globals['index_url'].'act=active">'.$l['active_users'].'</a>' : $l['active_users'] ).'
</td>
</tr>
...
<tr>
<td align="center" class="miposts" width="5%"><img src="'.$theme['images'].'online.gif" /></td>
<td class="mifor"><b><a href="'.$globals['index_url'].'act=members">'.$l['members'].'</a></b><br />
'.$l['list_of_members'].
...
<tr>
<td align="center" class="miposts" width="5%">
<img src="'.$theme['images'].'stats.gif" />
</td>
<td class="mifor">
...
'.$l['welcome_new_member'].', <b><a href="'.$globals['index_url'].'mid='.$latest_mem[1].'" >'.$latest_mem[0].'</b>
</td>
</tr>
...
<td align="right" class="dwhc"><a href="javascript:hideel(\'newpm\')"><img src="'.$theme['images'].'close.gif"></a></td>
<td class="dwhr"></td>
</tr>
</table>
...
<tr>
<td style="border-bottom:solid 1px #CCCCCC;">'.$l['pm_subject'].' <a href="'.$globals['index_url'].'act=usercp&ucpact=showpm&pmid='.$pm['pmid'].'">'.$pm['pm_subject'].'</a>
</td>

<td style="border-bottom:solid 1px #CCCCCC;" align="right">
'.$l['pm_sender'].' <a href="'.$globals['index_url'].'mid='.$pm['id'].'">'.$pm['sender'].'</a>
</td>
</tr>
